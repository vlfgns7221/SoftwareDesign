    ...
    def handle_mouse_event(self,event,turn,stone_color):
        if event.type == pygame.MOUSEBUTTONUP:
            ...
            if posx >= 0 and posx < 14 and posy >= 0 and posy < 14 and self.board.check_existance(self.model,k):
                ...
                return turn.change_stone_color(stone_color)
            else:
                return stone_color

class Change_Turn:
    def change_stone_color(self, stone_color):
        if stone_color==0:
            return 1
        elif stone_color==1:
            return 0
...
class Start_Button_Checker:
    # return true when clicking start button or when restart = TRUE
    def play_start_button(self,event,restart):
        if event.type == MOUSEBUTTONUP:
                pos = pygame.mouse.get_pos()
                if ~restart and pos[0] > 500 and pos[0] < 700 and pos[1] > 0 and pos[1] < 100:
                    return True
                elif restart:
                    return True
                else:
...
def main(check_run,restart):
    ...
    turn=Change_Turn();
    stone_color=turn.change_stone_color(0)  # Black Start
    ...
    while check_run_starter:
        for event in pygame.event.get():
            ...
            if restart or event.type == MOUSEBUTTONUP:  # if restart or put stone
                check_run=startbuttonchecker.play_start_button(event,restart)
                while check_run:
                    for event in pygame.event.get():
                        if event.type == QUIT:
                            check_run = False
                            check_run_starter= False
                        if event.type == MOUSEBUTTONUP:
                            startbuttonchecker.play_restart_button(event)   # check if restart button pressed
